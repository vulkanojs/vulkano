...
- Markup repeated across 2+ views → extract to `app/views/_shared/partials/` and `{% include %}` it, instead of duplicating.
...
{% if pagination.prev %}<a href="?page={{ pagination.prev }}">Prev</a>{% endif %}
...
```html
{{ vite({ entry: 'app', type: 'style' }) | safe }}   <!-- in <head> -->
{{ vite({ entry: 'app', type: 'script' }) | safe }}  <!-- before </body> -->
```

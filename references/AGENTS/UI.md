...
Task needs a pre-built UI component (dialog, dropdown, table, etc.) — pick and install the library at the project level (see `PROJECT.md`); the library itself (shadcn-vue, Element Plus, PrimeVue, or any other) is a project decision, not a framework one. Whatever library is chosen, these rules always apply:

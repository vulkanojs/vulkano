...
- **Entry points**: `vite.entries.mjs` maps one key per entrypoint — this template ships 2 by default (`{ app: 'frontend/website/app.js', admin: 'frontend/admin/app.js' }`). `vite.config.mjs` imports this map and derives both `build.rollupOptions.input`/`environments` and `resolve.alias` from it — no entrypoint wiring lives in `vite.config.mjs` itself. With only 1 entrypoint, `frontend/` stays flat (no subfolder) and the map holds a single key. Adding a 2nd entrypoint is the trigger to restructure `frontend/` into a container: the existing app moves to `frontend/website/`, each new one gets its own `frontend/<name>/` (e.g. `frontend/admin/`), and `vite.entries.mjs` gains one key per app — see reference/ARCHITECTURE.md § Multiple entry points and `.claude/skills/vulkano-skills/vulkano-frontend-entrypoint/SKILL.md`. The Nunjucks `vite()` helper takes an `entry` param (`vite({ entry: 'app', type: 'script' })`), so wiring a new bundle into a template only needs the matching `entry:` value — no other config changes.
- **Output**: assets land in `public/js/`, `public/css/`, `public/img/` — served directly by Express (`outDir: public/`, `emptyOutDir: false` so backend-served files aren't wiped). A production `vp build` runs each entry as its own isolated Rolldown pass (Vite's Environment API — `environments` + `builder.buildApp` in `vite.config.mjs`, with `consumer: 'client'` on each so Vite's CSS plugin still runs), so no entry ever imports a chunk shared with another entry, then merges every entry's own manifest into the single `public/.vite/manifest.json` Vulkano's `vite()` view helper reads.
...
- **Alias**: one per entrypoint, derived automatically from `vite.entries.mjs` — `@<dir>` for whatever folder that entry's path lives under (e.g. `frontend/website/app.js` → `@website`, `frontend/admin/app.js` → `@admin`). Adding a key to `vite.entries.mjs` is the only step; never reach across entrypoints through another one's alias (`@website` from inside `frontend/admin/`) — see `.claude/skills/vulkano-skills/vulkano-frontend-entrypoint/SKILL.md`.
- **Manifest**: `vite-plugin-dev-manifest` writes `public/.vite/manifest.<NODE_ENV>.json` in dev; production's merged `public/.vite/manifest.json` (see Entry points above) is what the Nunjucks templates read to inject the correct `<script>`/`<link>` tags in both cases
